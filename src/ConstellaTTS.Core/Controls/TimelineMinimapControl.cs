...
public sealed class TimelineMinimapControl : Control
{
    ...
    /// <summary>
    /// Optional history recorder. Set by the hosting view via
    /// <see cref="SetViewportRecorder"/> once DI has resolved the
    /// shared singleton. Null-tolerant: a minimap embedded in a
    /// previewer or test rig without a recorder still pans correctly,
    /// it just doesn't contribute to the undo stack. Production wiring
    /// is in TrackListView.Setup, which calls SetViewportRecorder
    /// alongside the existing SetTracks call.
    /// </summary>
    private IViewportHistoryRecorder? _recorder;
    ...
    /// <summary>
    /// Inject the application's <see cref="IViewportHistoryRecorder"/>
    /// so that pan and select-range gestures contribute to the undo
    /// stack. Called from the hosting view's DI wiring; safe to skip
    /// (e.g. in a previewer) at the cost of losing history coverage
    /// for minimap-driven viewport changes.
    /// </summary>
    public void SetViewportRecorder(IViewportHistoryRecorder recorder)
    {
        _recorder = recorder;
    }
    ...
    private void OnPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        // Open / extend a viewport-history session at the start of any
        // gesture that's about to mutate the viewport. The recorder
        // snapshots the current state as the FROM anchor; the eventual
        // settle (after the burst quietens) pushes a single
        // ViewportChangeAction. Same recorder is shared with the track
        // canvas wheel handler, so a minimap pan followed by a wheel
        // tweak both contribute to the same history coalescing.
        _recorder?.Touch();
    ...
    private void OnMoved(object? sender, PointerEventArgs e)
    {
        ...
        {
            // Each frame of a Pan drag is another viewport mutation.
            // Touching here keeps the recorder's settle timer alive
            // throughout the drag so the eventual commit captures the
            // full FROM→TO range; without this the timer could fire
            // mid-drag and produce two undo entries for one continuous
            // user gesture.
            _recorder?.Touch();
    ...
    private void OnReleased(object? sender, PointerReleasedEventArgs e)
    {
        if (_dragMode == DragMode.Select)
        {
            ...
            if (en - s > 0.5)
            {
                // Touch immediately before the FitViewportTo mutation
                // so the recorder captures pre-fit zoom + scroll as the
                // FROM. The Touch issued in OnPressed already opened a
                // session, but its FROM was identical (we hadn't
                // mutated yet); calling Touch again is harmless
                // (Touch on an active session is a no-op apart from
